...
var COORDINATION_NUMBER = 'samordningsnummer';
var PERSONAL_NUMBER = 'personnummer';
...
function dateToStr(date, opts, coordinationNumber){
  opts = opts || {};
  var day = (date.getDate() + (coordinationNumber ? 60 : 0));
  return (
...
function getBirthDate(century, date, delimiter){
  ...
  // Normalize day if coordination number
  if (day > 60) day = day - 60;
...
function getType(dateString) {
  var day = parseInt(dateString.slice(4,6), 10);

  // Coordniation numbers have the day incremented by 60
  if (day > 60) return COORDINATION_NUMBER;
...
function parse(pernr){
  ...
  return {
    ...
    number: parsed[4],
    type: getType(parsed[2]),
  };
}

function rand(min, max) { // both inclusive
	var num = Math.floor(Math.random() * (max - min + 1)) + min;
	return (num.toString().length === 1)? `0${ num }` : num;
};

function RandomPernr() {
	var year = rand(0, 99);
	var mo = rand(1, 12);
	var day = rand(1, 28);
	var lastThreeDigits = rand(111, 999);

	var partialBuild = `${ year }${ mo }${ day }${ lastThreeDigits }`;
	var lastDigit = getChecksum(partialBuild.toString().split(''));
	return `${ partialBuild }${ lastDigit }`;
};
...
Pernr.prototype.isCoordinationNumber = function(){
  return this.type == COORDINATION_NUMBER;
};
...
Pernr.prototype.toString = function(opts){
  ...
  return dateToStr(this.birthDate, opts, this.isCoordinationNumber()) + delim+ this.number;
};

Pernr.MALE = MALE;
Pernr.FEMALE = FEMALE;
Pernr.COORDINATION_NUMBER = COORDINATION_NUMBER;
Pernr.PERSONAL_NUMBER = PERSONAL_NUMBER;
